...
class FlowAugmentor:
    def __init__(self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=True, yjitter=False, saturation_range=[0.6,1.4], gamma=[1,1,1,1], stretch=False):
    ...
    def spatial_transform(self, img1, img2, flow, sim_flow, sim_valid):
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            sim_flow, sim_valid = self.resize_sparse_flow_map(sim_flow, sim_valid, fx=scale_x, fy=scale_y)
        ...
        if self.yjitter:
            ...
            sim_flow = sim_flow[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
            sim_valid = sim_valid[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        ...
        return img1, img2, flow, sim_flow, sim_valid


    def __call__(self, img1, img2, flow, sim_flow, sim_valid):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        img1, img2, flow, sim_flow, sim_valid = self.spatial_transform(img1, img2, flow, sim_flow, sim_valid)

        img1 = np.ascontiguousarray(img1)
        img2 = np.ascontiguousarray(img2)
        flow = np.ascontiguousarray(flow)
        sim_flow = np.ascontiguousarray(sim_flow)
        sim_valid = np.ascontiguousarray(sim_valid)
...
class SparseFlowAugmentor:
    ...
    def resize_sparse_flow_map(self, flow, valid, sim_flow, sim_valid, fx=1.0, fy=1.0):
        ht, wd = flow.shape[:2]
        coords = np.meshgrid(np.arange(wd), np.arange(ht))
        coords = np.stack(coords, axis=-1)
        ...
        sim_flow = sim_flow.reshape(-1, 2).astype(np.float32)
        sim_valid = sim_valid.reshape(-1).astype(np.float32)

        coords0 = coords[valid>=1]
        flow0 = flow[valid>=1]

        coords0_sim = coords[sim_valid>=1]
        flow0_sim = sim_flow[sim_valid>=1]
        ...
        coords1_sim = coords0_sim * [fx, fy]
        flow1_sim = flow0_sim * [fx, fy]

        xx = np.round(coords1[:,0]).astype(np.int32)
        yy = np.round(coords1[:,1]).astype(np.int32)

        xx_sim = np.round(coords1_sim[:,0]).astype(np.int32)
        yy_sim = np.round(coords1_sim[:,1]).astype(np.int32)
        ...
        v_sim = (xx_sim > 0) & (xx_sim < wd1) & (yy_sim > 0) & (yy_sim < ht1)
        xx_sim = xx_sim[v_sim]
        yy_sim = yy_sim[v_sim]
        flow1_sim = flow1_sim[v_sim]

        flow_img = np.zeros([ht1, wd1, 2], dtype=np.float32)
        valid_img = np.zeros([ht1, wd1], dtype=np.int32)

        sim_flow_img = np.zeros([ht1, wd1, 2], dtype=np.float32)
        sim_valid_img = np.zeros([ht1, wd1], dtype=np.int32)

        flow_img[yy, xx] = flow1
        valid_img[yy, xx] = 1

        sim_flow_img[yy_sim, xx_sim] = flow1_sim
        sim_valid_img[yy_sim, xx_sim] = 1

        return flow_img, valid_img, sim_flow_img, sim_valid_img

    def spatial_transform(self, img1, img2, flow, valid, sim_flow, sim_valid):
        # randomly sample scale

        ht, wd = img1.shape[:2]
        min_scale = np.maximum(
            (self.crop_size[0]) / float(ht),  #+1
            (self.crop_size[1]) / float(wd))  #+1

        scale = 2 ** np.random.uniform(self.min_scale, self.max_scale) # default [0.87 ~ 1.32]
        scale_x = np.clip(scale, min_scale, None)
        scale_y = np.clip(scale, min_scale, None)

        if True or np.random.rand() < self.spatial_aug_prob:
            # rescale the images
            img1 = cv2.resize(img1, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            img2 = cv2.resize(img2, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            flow, valid, sim_flow, sim_valid = self.resize_sparse_flow_map(flow, valid, sim_flow, sim_valid, fx=scale_x, fy=scale_y)

        if self.do_flip:
            rand1 = np.random.rand()
            if rand1 < self.h_flip_prob and self.do_flip == 'hf': # h-flip
                img1 = img1[:, ::-1]
                img2 = img2[:, ::-1]
                flow = flow[:, ::-1] * [-1.0, 1.0]
                sim_flow = sim_flow[:, ::-1] * [-1.0, 1.0]
            ...
            if rand2 < self.h_flip_prob and self.do_flip == 'h': # h-flip for stereo
                tmp = img1[:, ::-1]
                img1 = img2[:, ::-1]
                img2 = tmp

            rand3 = np.random.rand()
            if rand3 < self.v_flip_prob and self.do_flip == 'v': # v-flip
                img1 = img1[::-1, :]
                img2 = img2[::-1, :]
                flow = flow[::-1, :] * [1.0, -1.0]
                sim_flow = sim_flow[::-1, :] * [1.0, -1.0]
        ...
        sim_flow = sim_flow[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        sim_valid = sim_valid[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        return img1, img2, flow, valid, sim_flow, sim_valid


    def __call__(self, img1, img2, flow, valid, sim_flow, sim_valid):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        img1, img2, flow, valid, sim_flow, sim_valid = self.spatial_transform(img1, img2, flow, valid, sim_flow, sim_valid)
