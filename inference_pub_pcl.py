...
class D3RoMa():
    ...
    def run_pipeline(self, left_image, right_image, raw_disp, rgb):
        ...
        time4 = time.time()
        print(f"run pipeline function time_diff: {time4 - time3}")
...
if __name__ == "__main__":
    ...
    # pub = rospy.Publisher('fused_pcl', PointCloud2, queue_size=10)
    ...
    # 初始化相机
    camera_config = Realsense.default_real("d435")
    overrides = [
    ...
    rgb_sub = rospy.Subscriber(
        "/camera/color/image_raw",  # 根据实际话题名称修改
        Image,
        rgb_callback
    )
    
    depth_sub = rospy.Subscriber(
        "/camera/depth/image_rect_raw",  # 根据实际话题名称修改
        Image,
        depth_callback
    )
